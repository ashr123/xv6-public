...
void test(int testId, string got, string expected, vector <string> args) {
	...
	} else if (expected == "$$$ ASSERT_NO_THROWN_EXCEPTIONS $$$" || expected == "$$$ DECLARE GOOD TEST $$$" ||
	           got == expected)
		testPassed = true;
...
int pcloseMonitor(pid_t monitorPID) {
	...
	if ((waitReturnBeforeKill > 0 && WIFEXITED(monitorStatusBeforeKill) && WEXITSTATUS(monitorStatusBeforeKill) == 0) ||
		...
		returnStatus = 1;
	} else if ((waitReturnBeforeKill > 0 && WIFEXITED(monitorStatusBeforeKill) &&
	            WEXITSTATUS(monitorStatusBeforeKill) == 2) ||
	           (waitReturnAfterKill > 0 && WIFEXITED(monitorStatusAfterKill) &&
	            WEXITSTATUS(monitorStatusAfterKill) == 2)) {
		returnStatus = 0;
	} else {
		returnStatus = -1;
	}
