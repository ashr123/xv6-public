...
public:
	...
	bool
	enqueue(Proc *p); //append the given proc to the end of the list. Allocates a link node. Returns false if the allocation falied.
	Proc *
	dequeue();       //removes and returns the first proc of this linked list. Deallocates a link node. Returns null if this list is empty().

	bool remove(Proc *p); //remove a specific proc from this list. Returns true iff succeeds.

	bool
	transfer();                 //transfers all the procs to the Priority Queue. Fails if allocations failed. Deallocates link nodes.
...
private:
	...
	void forEach(const Func &accept)
	{ //for-each loop. gets a function that applies the procin each link node.
		Link *link = first;
		while (link)
		{
			accept(link->p);
...
public:
	...
	bool
	put(Proc *p);                 //puts the give proc in the rooted tree. Allocates a map node if needed. Allocates a link node. Returns true iff succeeds.
	...
	Proc *
	dequeue();                 //removes and returns the first proc of this->listOfProcs. Deallocates a link node. Returns null if this->listOfProcs is empty().
...
public:
	...
	bool
	put(Proc *p);                 //puts the give proc in this->root node. Allocates a map node if needed. Allocates a link node. Returns true iff succeeds.
	bool getMinKey(
			long long *pkey); //stores the minmum key of this rooted tree in the pkey arg. Returns true iff this map isn't empty.
	Proc *
	extractMin();                 //removes and returns a minimum proc from this map. Deallocates a map node if needed. Deallocates a link node. Returns null if this map is empty().
	bool
	transfer();                 //transfers all the procs to the Round Robin Queue. Fails if allocations failed. Deallocates map nodes. Deallocates link nodes.
