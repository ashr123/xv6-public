...
void initSchedDS()
{ //called once by the "pioneer" cpu from the main function in main.c
	data = null;
	spaceLeft = 0u;
...
bool MapNode::put(Proc *p)
{ //we can not use recursion, since the stack of xv6 is too small....
	MapNode *node = this;
	long long key = getAccumulator(p);
	for (;;)
	{
