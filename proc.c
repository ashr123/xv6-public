...
void pinit(void) {
	...
	for (struct kthread_mutex_t *m = kthread_mutexs; m < &kthread_mutexs[MAX_MUTEXES]; m++) {
		initlock(&m->lk, "lock");
		m->tid = 0;
		m->held = 0;
		m->locked = 0;
...
kthread_join(int tid) {
	...
	struct proc *p = myproc();
	for (struct thread *t = p->threads; t < &p->threads[NTHREAD]; t++) {
		if (t->tid == tid && t->state != THREAD_UNUSED) {
			_check:
			if (t->state != THREAD_UNUSED && t->state != THREAD_ZOMBIE) {
				sleep(t, &ptable.lock);
				goto _check;
			}
			if (t->state == THREAD_ZOMBIE) {
...
int kthread_mutex_lock(int mutex_id) {
	...
	kthread_mutexs[mutex_id].locked = 1;
	release(&kthread_mutexs[mutex_id].lk);
	return 0;
}
...
int kthread_mutex_unlock(int mutex_id) {
	...
	kthread_mutexs[mutex_id].tid = 0;
	kthread_mutexs[mutex_id].locked = 0;
	release(&kthread_mutexs[mutex_id].lk);
	wakeup(&kthread_mutexs[mutex_id]);
	return 0;
